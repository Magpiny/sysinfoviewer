...
MyApp::OnInit()
{
  ...
  private:
    ...
    void UpdateFontSizes()
    {
      ...
      int dynamicSize =
        wxMax(m_baseFontSize - 2, wxMin(m_baseFontSize + 2, windowWidth / 50));
      wxFont font = GetFont();
            ...
            if (size_re.Matches(line)) {
              ...
              if (width_mm > 0 && height_mm > 0) {
                double diagonal_inches =
                  std::sqrt(width_mm * width_mm + height_mm * height_mm) / 25.4;
  ...
  : wxFrame(nullptr, wxID_ANY, "Sytem Information")
{
  ...
  menuFile->Append(
    ID_Hello, "&Hello...\tCtrl-I", "Show system information summary");
...
MyFrame::OnHello(wxCommandEvent& event)
{
  ...
  wxMessageBox(wxString::Format(
                 "User: %s\nDate: %s \nOS: %s", userName, currentTime, osDesc),
               title,
               wxOK | wxICON_INFORMATION);
}
