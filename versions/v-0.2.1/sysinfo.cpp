...
MyApp::OnInit()
{
  ...
  wxTreebook* treebook = new wxTreebook(
    frame, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxNB_LEFT);
  ...
  //.................................... CPU USAGE
  //...............................................//
  wxStaticText* cpuUsageTitle = new wxStaticText(systeminfoPage,
                                                 wxID_ANY,
                                                 "CPU USAGE",
                                                 wxDefaultPosition,
                                                 wxDefaultSize,
  ...
  {
  public:
    CPUDoughnutChartPanel(wxWindow* parent)
      : wxPanel(parent)
      , m_cpuUsage(0.0)
    {
  ...
  private:
    ...
    void DrawDoughnutChart(wxGraphicsContext* gc)
    {
      ...
      unusedPath.AddArc(
        centerX, centerY, outerRadius, usedAngle, 2 * M_PI, true);
      unusedPath.AddArc(
        centerX, centerY, innerRadius, 2 * M_PI, usedAngle, false);
      ...
      gc->DrawText(
        usageText, centerX - textWidth / 2, centerY - textHeight / 2);

      // Add CPU info below the chart
      wxString cpuInfo = wxString::Format(
        "CPU Count: %d | CPU Speed: %.2f MHz", m_cpuCount, m_cpuSpeed);
      gc->SetFont(
        wxFont(
  ...
  topRowCenterChild->Add(
    new wxStaticText(systeminfoPage, wxID_ANY, "MEMORY USAGE"),
    0,
  ...
  {
  public:
    MemoryDoughnutChartPanel(wxPanel* parent)
  ...
  private:
    ...
    void DrawDoughnutChart(wxGraphicsContext* gc)
    {
      ...
      gc->DrawEllipse(centerX - innerRadius,
                      centerY - innerRadius,
                      innerRadius * 2,
                      innerRadius * 2);
      ...
      gc->DrawText(
        memoryInfo, centerX - textWidth / 2, centerY - textHeight / 2);
    ...
    void DrawLabel(wxGraphicsContext* gc,
  ...
  topRowRightChild->Add(
    new wxStaticText(systeminfoPage, wxID_ANY, "DISK USAGE"),
    0,
    wxALL | wxALIGN_CENTRE_HORIZONTAL,
    5);
  ...
  {
  public:
    DiskUsagePieChart(wxWindow* parent)
      : wxPanel(parent)
    {
  ...
  private:
    ...
    void DrawPieChart(wxGraphicsContext* gc)
    {
      ...
      DrawLabel(
        gc, "Used: " + FormatSize(m_usedSpace), centerX - radius, height - 40);
      DrawLabel(
        gc, "Free: " + FormatSize(m_freeSpace), centerX + radius, height - 40);
      ...
      gc->DrawText(
        percentageText, centerX - textWidth / 2, centerY - textHeight / 2);
    }
  ...
  bottomRowLeftChild->Add(
    new wxStaticText(systeminfoPage, wxID_ANY, "OS INFORMATION"), 0, wxALL, 5);
  ...
  bottomRowRightChild->Add(
    new wxStaticText(systeminfoPage, wxID_ANY, "BATTERY INFORMATION"),
    0,
    wxALL | wxALIGN_CENTRE_HORIZONTAL,
    5);
  ...
  {
  public:
    BatteryInfoPanel(wxPanel* parent)
      : wxPanel(parent)
    {
      SetBackgroundStyle(wxBG_STYLE_PAINT);
  ...
  private:
    ...
    void OnPaint(wxPaintEvent& event)
    {
      ...
      if (gc) {
        ...
        gc->DrawRectangle(
          barX, barY + barHeight - filledHeight, barWidth, filledHeight);
  ...
  public:
      ...
      : wxPanel(parent, wxID_ANY)
    {
      ...
      m_listCtrl->Bind(
        wxEVT_LIST_ITEM_ACTIVATED, &ResourcesInfoPage::OnItemActivated, this);
  ...
  private:
    ...
    void RefreshProcessList()
    {
      ...
      for (size_t i = 1; i < output.GetCount(); ++i) {
        ...
        if (fields.size() >= 11) {
          ...
          std::copy(fields.begin() + 10, fields.end(), std::back_inserter(commandParts));
    ...
    {
      wxMessageBox(
        wxString::Format("Total number of processes: %zu", m_processes.size()),
        "Process Count",
  ...
  {
  public:
    NetworkInfoPanel(wxWindow* parent)
      : wxPanel(parent, wxID_ANY)
    {
      wxBoxSizer* mainSizer = new wxBoxSizer(wxVERTICAL);
  ...
  private:
    ...
    void GetInterfaceInfo(std::string& interfaceName,
    ...
    void GetNetworkUsage(const std::string& interface,
                         long long& txBytes,
                         long long& rxBytes)
    {
  ...
  {
  public:
    DisplayInfoPanel(wxWindow* parent)
      : wxPanel(parent, wxID_ANY)
    {
  ...
  private:
    ...
    void CollectFromWxDisplay()
    {
      ...
      if (displayCount > 0) {
        ...
        m_displayInfo.emplace_back(
          "Screen Resolution",
          std::format("{}x{}", mode.GetWidth(), mode.GetHeight()));
        m_displayInfo.emplace_back(
          "Refresh Rate", std::format("{} Hz", 60)); // mode.refresh =! 60
    ...
    void CollectDisplayManager()
    {
      ...
      wxArrayString output;
      if (wxExecute("ps -e | grep -E 'gdm|sddm|lightdm|lxdm|xdm'",
                    output,
                    wxEXEC_NODISABLE) == 0) {
        if (!output.IsEmpty()) {
          ...
          if (line.Contains("gdm"))
            displayManager = "GDM";
          else if (line.Contains("sddm"))
            displayManager = "SDDM";
          else if (line.Contains("lightdm"))
            displayManager = "LightDM";
          else if (line.Contains("lxdm"))
            displayManager = "LXDM";
          else if (line.Contains("xdm"))
            displayManager = "XDM";
        }
      }
    ...
    void CollectFromEdid()
    {
      try {
        ...
        for (const auto& entry :
             std::filesystem::directory_iterator(edidPath)) {
          ...
          if (std::filesystem::exists(filename)) {
            ...
            if (file) {
              ...
              if (file.read(reinterpret_cast<char*>(buffer.data()), 128)) {
                ...
                manufacturer +=
                  static_cast<char>(((buffer[8] & 0x7C) >> 2) + 'A' - 1);
                manufacturer += static_cast<char>(
                  ((buffer[8] & 0x03) << 3 | (buffer[9] & 0xE0) >> 5) + 'A' -
                  1);
                ...
                m_displayInfo.emplace_back("Monitor Manufacturer",
                                           manufacturer);
    ...
    {
      wxArrayString output;
      if (wxExecute("lspci | grep -i vga", output, wxEXEC_NODISABLE) == 0 &&
          !output.IsEmpty()) {
        m_displayInfo.emplace_back(
          "GPU Model", output[0].AfterFirst(':').Trim().ToStdString());
      }

      std::ifstream file("/sys/class/drm/card0/device/driver/module/drivers/"
                         "pci:*/module/version");
      if (file) {
        std::string version;
    ...
    void CreateControls()
    {
      for (const auto& info : m_displayInfo) {
        auto* label = new wxStaticText(
          this, wxID_ANY, wxString::Format("%s: %s", info.first, info.second));
  ...
  // ---------------------------- STORAGE DEVICES
  // ---------------------------------------------
  wxStaticBoxSizer* storageSizer =
    new wxStaticBoxSizer(wxVERTICAL, storageDevicesPane, "STORAGE INFORMATION");

  class CustomGauge : public wxPanel
  {
  public:
    CustomGauge(wxWindow* parent,
  ...
  public:
      ...
      : wxPanel(parent, id)
    {
      ...
      wxLogMessage(wxString::Format(
        "Panel size: %d x %d", GetSize().GetWidth(), GetSize().GetHeight()));
    }
      ...
      : wxPanel(parent, id)
    {
      ...
      if (audioDevices.empty()) {
        ...
        for (const auto& device : audioDevices) {
          ...
          deviceSizer->Add(
            new wxStaticText(scrolledWindow, wxID_ANY, "Type: " + typeStr),
            0,
            wxALL,
            2);
          deviceSizer->Add(
            new wxStaticText(
              scrolledWindow, wxID_ANY, "Description: " + device.description),
            0,
            wxALL,
            2);

          wxString formatStr =
            wxString::Format("Format: %s, %d channels, %d Hz",
                             device.sampleFormat.c_str(),
                             device.channels,
                             device.sampleRate);
          deviceSizer->Add(
            new wxStaticText(scrolledWindow, wxID_ANY, formatStr), 0, wxALL, 2);
      ...
      while (snd_card_next(&cardNum) >= 0 && cardNum >= 0) {
        ...
        while (snd_ctl_pcm_next_device(cardHandle, &deviceNum) >= 0 &&
          ...
          for (int isPlayback = 0; isPlayback <= 1; ++isPlayback) {
            ...
            info.name = wxString::Format(
              "%s (Card %d, Device %d)", cardName, cardNum, deviceNum);
            ...
            char deviceIdStr[64];
            snprintf(
              deviceIdStr, sizeof(deviceIdStr), "hw:%d,%d", cardNum, deviceNum);

            if (snd_pcm_open(&pcmHandle,
                             deviceIdStr,
                             isPlayback ? SND_PCM_STREAM_PLAYBACK
                                        : SND_PCM_STREAM_CAPTURE,
                             SND_PCM_NONBLOCK) >= 0) {
  ...
  private:
    ...
    void PopulateMotherboardInfo()
    {
      ...
      for (const auto& [key, value] : info) {
        ...
        wxStaticText* valueLabel = new wxStaticText(this,
                                                    wxID_ANY,
                                                    value,
                                                    wxDefaultPosition,
                                                    wxDefaultSize,
  ...
  {
  public:
    CustomCPUCoreGauge(wxWindow* parent,
                       wxWindowID id = wxID_ANY,
                       int range = 100,
  ...
  private:
    ...
    std::vector<std::pair<wxString, wxString>> GetCPUInfo()
    {
      ...
      info.emplace_back("Architecture:", "64 bit"); // hardcoded
    ...
    void CreateCoreGauges()
    {
      ...
      for (int i = 0; i < m_numCores; ++i) {
        ...
        wxStaticText* coreLabel =
          new wxStaticText(this, wxID_ANY, wxString::Format("Core %d: 0%%", i));
  ...
  {
  public:
    ApplicationsPanel(wxWindow* parent)
      : wxPanel(parent, wxID_ANY)
    {
      try {
        CreateControls();
...
MyFrame::MyFrame()
  : wxFrame(nullptr, wxID_ANY, "Sytem Information")
{
...
{
  wxMessageBox("This application is running under %s.\n \
      Toolkit Type: wxGTK3\n \
      Toolkit Used: 3.2.5",
